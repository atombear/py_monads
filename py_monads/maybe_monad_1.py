...
if __name__ == "__main__":
    ...
    assert bind(
        bind(Just(3), Kleisli(lambda x: Just(x + 5))), Kleisli(lambda x: Just(x * 3))
    ) == Just(24)
    ...
    assert bind_chain(
        ...
        Kleisli(lambda x: Just(x - 11)),
    ) == Just(40)

    assert (
        bind_chain(
            Just(1), Kleisli(lambda x: Just(x / 0)), Kleisli(lambda x: Just(x + 3))
        )
        == Nothing()
    )

    assert (
        Just(12) * Kleisli(lambda x: Just(x + 10) * Kleisli(lambda x: Just(x * 3)))
    ) == Just(66)

    assert (
        Just(30) * (Kleisli(lambda x: Just(x / 10)) * Kleisli(lambda x: Just(x + 3)))
    ) == Just(6.0)
